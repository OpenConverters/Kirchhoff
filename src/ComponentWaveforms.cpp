...
// equal the deck's. (Kept a local copy: sanitize() lives in the assembler's anonymous namespace.)
...
InterStage read_inter_stage(const json& topo) {
    InterStage is;
...
BrickNets read_brick_nets(const json& brick) {
    BrickNets bn;
...
std::string node_of_pin(const std::string& stage, const std::string& comp, const std::string& pin,
...
std::vector<double> node_voltage(const std::string& node, const NgspiceRunResult& r, bool& ok) {
    ok = true;
