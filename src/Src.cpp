...
json build_src_tas(const SrcDesign& d) {
    ...
    if (negativeRail && (d.rectifierType == RectifierType::CurrentDoubler ||
        ...
        throw std::invalid_argument(
            "Kirchhoff SRC: a negative output rail is only supported for the centerTapped and fullBridge "
    ...
    // relabelled — nothing is reversed, so every variant mirrors without variant-specific code.
