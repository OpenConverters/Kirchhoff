...
//
// createKirchhoff() builds one engine instance. Importing this module does nothing: no worker, no
// fetch, no global. Every location the engine reaches for is an option of the instance —
//   wasmUrl              the emscripten module (kirchhoff.js) the worker imports. Required.
//   kelvinUrl            base URL of the Kelvin shards + NDJSON catalogues (<kelvinUrl>/manifest.json,
//                        <kelvinUrl>/<family>.kidx|.ndjson). null disables Kelvin: the sourcing calls
//                        throw instead of fetching.
//   openMagneticsOrigin  the OpenMagnetics app the magnetic-adviser handoff opens.
//   telemetry            false (default) sends nothing; true forwards product events to the
//                        OpenConverters telemetry sink (telemetry.js — which itself stays inert until the
//                        host calls initTelemetry); a function (eventType, props) receives them instead.
//   createWorker         () => Worker running src/worker.js. Required here: how the worker file is
...
// Kelvin family per BOM kind (bom.js `kind`). Pure data, so it is a plain export too.
...
export function createKirchhoff({
  wasmUrl,
  kelvinUrl = null,
  telemetry = false,
  openMagneticsOrigin = 'https://openmagnetics.com',
  createWorker = null,
} = {}) {
  if (typeof wasmUrl !== 'string' && !(wasmUrl instanceof URL))
    throw new Error('createKirchhoff: wasmUrl (the URL of kirchhoff.js) is required')
  if (kelvinUrl !== null && typeof kelvinUrl !== 'string' && !(kelvinUrl instanceof URL))
    throw new Error('createKirchhoff: kelvinUrl must be a URL string, or null to disable Kelvin')
  if (typeof createWorker !== 'function')
    throw new Error('createKirchhoff: createWorker (() => new Worker(…worker.js…)) is required')
  // Absolute before it crosses into the worker, whose own base URL (a file under assets/, or a blob:)
  // is not the page's: a relative wasmUrl means relative to the page.
  const WASM_URL = typeof document !== 'undefined' ? new URL(String(wasmUrl), document.baseURI).href : String(wasmUrl)
  const KELVIN_BASE = kelvinUrl === null ? null : String(kelvinUrl).replace(/\/+$/, '')
  const OPENMAGNETICS_ORIGIN = openMagneticsOrigin

  const track = typeof telemetry === 'function'
    ? (eventType, props) => { try { telemetry(eventType, props) } catch (_) { /* telemetry must never break the app */ } }
    : telemetry === true ? trackEvent : () => {}
  ...
  function getWorker() {
    if (!worker) {
      ...
      // First message: where the engine lives. Messages are handled in order, so every call below
      // reaches a worker that already knows it.
      worker.postMessage({ id: 0, fn: '__config__', args: [WASM_URL] })
    }
    return worker
  }
  ...
  function processConverter(topology, spec, engine = 'analytical') {
    return callJson('process_converter', topology, JSON.stringify(spec), engine)
  }

  function topologyWaveforms(tas) {
  ...
  function componentWaveforms(tas, fidelity = { origin: 'REQUIREMENTS' }) {
  ...
  function extractOperatingPoint(tas, engine = 'analytical', magneticName = '', fidelity = { origin: 'REQUIREMENTS' }) {
  ...
  function mainMagneticInputs(tas, ref = '') {
  ...
  function enrichMagneticWaveforms(inputs, windings) {
  ...
  // Origin of the OpenMagnetics app: the instance's openMagneticsOrigin option.
  ...
  async function designMagneticInOpenMagnetics(ref, inputs) {
  ...
  async function suggestMagneticsInOpenMagnetics(ref, inputs) {
  ...
  function bindMagnetic(tas, ref, mas) {
    const magnetic = mas?.magnetic ?? mas
    return callJson('bind_part', JSON.stringify(tas), ref, JSON.stringify({ magnetic }))
  }

  function generateNetlist(tas, flavor = 'ngspice', fidelity = { origin: 'REQUIREMENTS' }) {
  ...
  function simulateNgspice(tas, fidelity = { origin: 'REQUIREMENTS' }) {
  ...
  // Kelvin disabled (kelvinUrl: null) is a configuration, not a failure to paper over: anything that
  // would fetch a shard or a record throws, naming the option.
  function kelvinBase() {
    if (KELVIN_BASE === null) throw new Error('Kelvin is disabled for this Kirchhoff instance (createKirchhoff({ kelvinUrl: null })) — part sourcing needs a kelvinUrl')
    return KELVIN_BASE
  }
  ...
  function kelvinManifest() {
    if (!_manifestPromise) {
      _manifestPromise = (async () => {
        const res = await fetch(`${kelvinBase()}/manifest.json`, { cache: 'no-store' })
  ...
  async function fetchShardBytes(category, buildId) {
    ...
    const url = `${kelvinBase()}/${category}.kidx?b=${buildId}&v=${fmt}`
    ...
    // Unguarded, this put took the whole category down — Chrome refuses Cache entries
    // for large responses under some quota conditions, and every family above ~2 MB
    // (magnetic 12.5 MB, capacitor 31.7 MB) threw NetworkError here. On Kelvin, which
    // shares this loader, that showed as "0/12 shards" and no parts while the data sat
    // fetched and intact (Kelvin 3b5c32d, ABT #418).
    try {
      await cache.put(url, res.clone())
    } catch (e) {
      console.warn(`Kelvin shard '${category}' not cached (${e.name}); loading without cache`)
    }
    return new Uint8Array(await res.arrayBuffer())
  }
  ...
  async function selectCandidates(kind, requirements, context = {}) {
  ...
  async function bindPart(tas, ref, kind, candidate) {
    ...
    if (envelope === undefined) {
      ...
      const entry = await manifestEntry(category)
      const res = await fetch(`${kelvinBase()}/${category}.ndjson`,
  ...
  function crossReference(kind, original, candidates, { originalVerified = true, maxResults = 12 } = {}) {
    const category = kelvinCategoryFor(kind)
    if (!category) throw new Error(`no Kelvin category for kind '${kind}'`)
  ...
  // The instance. Also a Vue plugin: app.use(kh) makes it injectable (useKirchhoff() / the components).
  const instance = {
    options: Object.freeze({ wasmUrl: WASM_URL, kelvinUrl: KELVIN_BASE, openMagneticsOrigin: OPENMAGNETICS_ORIGIN,
                             telemetry: telemetry !== false }),
    kelvinEnabled: KELVIN_BASE !== null,
    track,
    ...
    terminate() {
      ...
      worker.terminate()
      worker = null
      for (const p of pending.values()) p.reject(new Error('Kirchhoff engine terminated'))
      pending.clear()
    },
    install(app) { app.provide(KIRCHHOFF_KEY, instance) },
  }
  return instance
}
