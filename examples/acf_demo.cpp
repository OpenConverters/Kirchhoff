// acf_demo — build the PSFB TAS, emit the ideal ngspice deck, run it, print Vout.
...
static std::string run_ngspice(const std::string& deck, const std::string& tag) {
    const std::string path = "/tmp/acf_demo_" + tag + ".cir";
...
int main(int argc, char** argv) {
    ...
    if (const char* pe = getenv("ACF_DUTY")) d.dutyCycle = std::stod(pe);
    std::cout << "design: n=" << d.turnsRatio << " Lm=" << d.magnetizingInductance*1e6
              << "uH Lo=" << d.outputInductance*1e6 << "uH Cc=" << d.clampCapacitance*1e6
