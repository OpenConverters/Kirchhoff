// fsbb_demo — build the PSFB TAS, emit the ideal ngspice deck, run it, print Vout.
...
static std::string run_ngspice(const std::string& deck, const std::string& tag) {
    const std::string path = "/tmp/fsbb_demo_" + tag + ".cir";
...
int main(int argc, char** argv) {
    const double Vin = argc > 1 ? std::stod(argv[1]) : 12.0;
    ...
    if (const char* pe = getenv("FSBB_DUTY")) d.dutyCycle = std::stod(pe);
    std::cout << "design: L=" << d.inductance*1e6 << "uH D=" << d.dutyCycle
