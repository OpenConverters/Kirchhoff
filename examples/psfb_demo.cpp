// psfb_demo — build the PSFB TAS, emit the ideal ngspice deck, run it, print Vout.
...
static std::string run_ngspice(const std::string& deck, const std::string& tag) {
    const std::string path = "/tmp/psfb_demo_" + tag + ".cir";
...
int main(int argc, char** argv) {
    ...
    if (const char* pe = getenv("PSFB_PHI")) d.phaseDeg = std::stod(pe);
    std::cout << "design: n=" << d.turnsRatio << " Lr=" << d.seriesInductance*1e6
              << "uH Lm=" << d.magnetizingInductance*1e6 << "uH Lo=" << d.outputInductance*1e6
              << "uH Deff=" << d.effectiveDuty << " phi=" << d.phaseDeg
              << " Rload=" << d.loadResistance << "\n";
