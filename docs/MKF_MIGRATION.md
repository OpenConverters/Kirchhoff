...
**There is exactly ONE enum type: `MAS::Topology`** (generated by quicktype into MAS.hpp — the `$def`
is reachable from `MAS::DesignRequirements`). The codebase uses that single enum directly; there is
**no separate `PEAS::Topology` C++ type** (an earlier attempt to introduce one — first generated, then
as an alias — was reverted: a second type, even an alias, was unwanted churn for zero functional gain,
since the schema already makes the values PEAS-owned). The taxonomy is PEAS-owned at the SCHEMA layer
(its values come from `PEAS/schemas/utils.json#/$defs/topology`, which MAS `$ref`s); the C++ enum just
happens to be emitted in the `MAS` namespace, and that is fine.

- **KH:** `src/Topology.hpp` includes `MAS.hpp` and exposes `using Topology = MAS::Topology`
  (`Kirchhoff::Topology` is a brevity name for the one enum, not a second type — guarded by a
  `static_assert(std::is_same<Kirchhoff::Topology, MAS::Topology>)`). The ~24 converter `.cpp` use it
  via that name; byte-identical JSON; full suite 134/32.
- **MKF:** uses `MAS::Topology` directly (its existing state) — no change needed. It already consumes
  the PEAS-owned values via the generated enum.
